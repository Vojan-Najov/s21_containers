...
class MultisetTest: public ::testing::Test {
 public:
	...
	bool MultisetEqual(const s21::multiset<Key>&lhs, const s21::multiset<Key>& rhs) {
	...
	template <typename Key>
	bool MultisetEqual(const s21::multiset<Key>&lhs, const std::multiset<Key>& rhs) {
		return lhs.empty() == rhs.empty() &&
					 lhs.size() == rhs.size() &&
					 lhs.verify() == 0 &&
					 std::equal(lhs.cbegin(), lhs.cend(), rhs.cbegin());
	...
	template <typename Key>
	bool MultisetEqual(const std::multiset<Key>&lhs, const s21::multiset<Key>& rhs) {
		return lhs.empty() == rhs.empty() &&
					 lhs.size() == rhs.size() &&
					 rhs.verify() == 0 &&
					 std::equal(lhs.cbegin(), lhs.cend(), rhs.cbegin());
